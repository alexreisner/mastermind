...
Commits are compared by content, not ID, so lines that start with a "-" show commits which exist in master but with a different commit ID, while lines that start with a "+" have no equivalent commit upstream.
