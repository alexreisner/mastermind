...
First, add it to the main menu. Go to System > Preferences > Main Menu > System Tools and put a checkmark next to Configuration Editor. Then launch it.
...
Install various commonly-used packages:
  ...
  sudo apt-get install -y ubuntu-restricted-extras rar p7zip ntp \
    ...
    xclip tofrodos ack-grep vim-gnome
...
=== png2ico
  ...
  tar xf png2ico-src-2002-12-08.tar.gz
...
Restore MySQL and PostgreSQL databases from dumpfiles in /var/backups. If you are installing onto a system with the exact same database versions as before you can instead restore the contents of the directories:
...
=== Files
  ...
  /etc/
    ...
    postgresql/8.4/main/pg_hba.conf
    samba/smb.conf
